...
if [[ -z "${SSL_TYPE:-}" ]]; then
    echo ""
    log "${YELLOW}Выберите тип SSL-сертификата:${NC}"
    echo "  1) Let's Encrypt (автоматическое получение)"
    echo "  2) Существующий сертификат (файлы .key, .crt/.cer, опционально ca.crt)"
    read -p "Ваш выбор (1/2): " ssl_choice
    case "$ssl_choice" in
        1) SSL_TYPE="letsencrypt" ;;
        2) SSL_TYPE="existing" ;;
        *) log "${RED}Неверный выбор. Завершение.${NC}"; exit 1 ;;
    esac
...
if [[ "$SSL_TYPE" == "existing" ]]; then
    ...
    log "${GREEN}Найдены файлы:${NC}"
    log "  Ключ: $KEY_FILE"
    log "  Сертификат: $CERT_FILE"
    [[ -n "$CA_FILE" ]] && log "  CA-цепочка: $CA_FILE"
    ...
    log "${GREEN}Сертификаты сконвертированы и сохранены в $SSL_TARGET${NC}"
fi
...
# 6.1 functions.php
create_php_file "$ADMIN_DIR/includes/functions.php" '<?php
...
# 6.2 auth.php
create_php_file "$ADMIN_DIR/includes/auth.php" '<?php
session_start();
...
# 6.3 login.php
create_php_file "$ADMIN_DIR/login.php" '<?php
session_start();
require_once __DIR__ . "/../config.php";
...
# 6.5 header.php
create_php_file "$ADMIN_DIR/includes/header.php" '<?php
...
if [[ -f "$INDEX_FILE" ]]; then
    ...
    # Создаём минимальный index.php с трекером и вызовом роутера
    cat > "$INDEX_FILE" <<EOF
<?php
require_once __DIR__ . "/config.php";
$TRACKER_PHP
...
if [[ ! -f "$NGINX_CONF" ]] || $FORCE_MODE; then
    if [[ "$SSL_TYPE" == "letsencrypt" ]]; then
        # Временная конфигурация на порту 80
        cat > "$NGINX_CONF" <<EOF
server {
    listen 80;
    listen [::]:80;
...
EOF
        ...
        # Конфигурация обновится автоматически certbot'ом
    else
        # Существующий сертификат – создаём конфиг с HTTPS
        cat > "$NGINX_CONF" <<EOF
server {
    listen 80;
    listen [::]:80;
...
echo ""
log "${GREEN}======================================================"
log "${GREEN}✅ CMS и сайт успешно установлены!${NC}"
log "${GREEN}======================================================"
echo ""
