...
# Версия: 1.1 (добавлены ENV_FILE и add_to_env)
# Содержит унифицированные функции логирования, создания файлов,
# работы с БД, запроса переменных и прогресс-бара.
# =====================================================================

# Определяем каталог, в котором находится сама библиотека
LIB_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
export ENV_FILE="${LIB_SCRIPT_DIR}/.env"
...
# Добавление/обновление переменной в .env файле
add_to_env() {
    local key="$1"
    local value="$2"
    local env_file="$ENV_FILE"
